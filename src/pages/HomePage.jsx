...
const HomePage = () => {
  ...
  const renderFood = (foodArr, type) => {
    ...
    return (
      <>
        <div
          id={type}
          className="pt-4 xl:pt-10 max-w-[1248px] mb-3 px-6 mx-auto"
        >
          ...
          <ul className="flex gap-4 flex-wrap justify-center md:justify-start">
            {foodArr &&
              foodArr
                ...
                .map((item, index) => (
                  <li
                    className="max-w-[240px] sm:max-w-[250px] md:max-w-[260px] w-full xl:max-w-[288px]  flex flex-col "
                    key={index}
                  >
                    <img
                      src={item.imgUrl}
                      className="w-full rounded-t-xl h-[200px] md:h-[220px]"
                      alt="food"
                    />
                    <div className="px-4 flex-grow flex flex-col justify-end rounded-b-xl pt-3 pb-4 border-[1px] border-gray-100">
                      <h3 className="text-[19px] mb-[7px] font-semibold">
                        {item?.name}
                      </h3>
                      {item?.description && (
                        <p className="text-[#48535B] text-[14px]">
                          {item?.description.slice(0, 52)}...
                        </p>
                      )}
                      <div className="flex xl:flex-row flex-col w-full mt-2 justify-between xl:items-center">
                        <p className="text-[19px] mb-3 xl:mb-0 font-bold">
                          {item?.price}{" "}
                          <span className="text-[14px] font-normal">so'm</span>
                        </p>
                        {findQuantity(item) ? (
                          <div className="flex justify-evenly xl:justify-start items-center border-[1px] border-[#51267D] rounded-3xl gap-3 text-[17px] font-semibold">
                            <button
                              onClick={() => handleDecreaseCart(item)}
                              className="py-[7px] pl-[18px] "
                            >
                              <FaMinus />
                            </button>
                          ...
                          <button
                            onClick={() => handleAddToCart(item)}
                            className="bg-[#51267D] font-semibold tracking-[0.5px] py-[10px] px-6 text-[14px] text-white rounded-3xl"
                          >
                            Qo'shish
                          </button>
                        )}
  ...
  return (
    <div>
      ...
      <div
        className={`${
          hasNavFood ? "w-full shadow-md sticky top-0 bg-white" : ""
        }`}
      >
        <div className="max-w-[1248px] gap-4 flex items-center px-6 mx-auto mt-4 lg:mt-8">
          <ul className="flex w-[1130px] py-2 items-center gap-3 overflow-x-auto scroll_hidden">
            {typeArr.map((type, index) => (
              <li
                className="px-4 py-2 rounded-xl hover:bg-[#F1EFF4] cursor-pointer text-[18px] whitespace-nowrap"
                onClick={() => {
